...
Class hierarchy with multiple dataclass_transforms
--------------------------------------------------
Initially this proposal supported decorating metaclasses with
``dataclass_transform``, but not normal classes. Now that we're
supporting ``dataclass_transform`` on normal classes, it's possible to
create a class hierarchy containing more than one
``dataclass_transform`` class. How should type checkers respond in
that case?

Some possibilities:

* Ignore this scenario / leave the behavior undefined.
* Report an error if a class has more than one ``dataclass_transform``
  in its hierarchy.
* Try to honor all of the ``dataclass_transform`` decorators.

Along similar lines, how should a type checker respond if a single
function/class has multiple instances of the ``dataclass_transform``
decorator?::
